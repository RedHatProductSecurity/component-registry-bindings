...
def _get_kwargs(
    *,
    client: Client,
    form_data: ProductVersion,
    multipart_data: ProductVersion,
    json_body: ProductVersion,
) -> Dict[str, Any]:
...
def _parse_response(*, response: httpx.Response) -> Optional[ProductVersion]:
    if response.status_code == 201:
        _response_201 = response.json()
        response_201: ProductVersion
        if isinstance(_response_201, Unset):
            response_201 = UNSET
        else:
            response_201 = ProductVersion.from_dict(_response_201)

        return response_201
    return None


def _build_response(*, response: httpx.Response) -> Response[ProductVersion]:
    return Response(
